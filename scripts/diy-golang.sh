...
#   - openwrt-18.06-k5.4（helloworld pin 到 xray-core v1.8.24，go.mod 要求 go 1.21）：
#       自带 golang < 1.21 过低；但 xray v1.8.24 的 sing 老依赖在 Go 1.26 上会触发
#       `net.errNoSuchInterface` 链接错误，故升到 Go 1.22（sbwml 22.x）最稳
#       （>=1.21 且不过新，避开老依赖链接错误）。
...
# 按分支选 Go 大版本：
#   - 18.06-k5.4（pin xray v1.8.24）：Go 1.22，避开新 Go 触发老 sing 依赖链接错误
#   - 23.05 / master（最新 xray v25/v26）：Go 1.26，满足 go.mod 的 1.25+/1.26 要求
case "$SRC_BRANCH" in
  openwrt-18.06-k*) GOLANG_BRANCH="22.x" ;;
  *)                GOLANG_BRANCH="26.x" ;;
esac
