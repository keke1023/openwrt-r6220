...
# 注：config-18.06/ 下现仅剩 dw33d（128MB NAND），故 18.06-k5.4 统一用 helloworld master 最新，
#     不再按机型 pin 旧版（k2p 等小闪存机型已删除，无体积限制）。
case "$SRC_BRANCH" in
...
#   - 18.06-k5.4：用 master 分支最新提交（ssr-plus 190-3+ / xray-core v26.5.9）。
#       xray v26.5.9 源码编译需 Go 1.26，由 diy-golang.sh 升级 Go 到 1.26。
#       不再 pin 旧 commit（k2p 等小闪存机型已删除，仅剩 dw33d 128MB，空间充足）。
...
# 全量 clone（非浅克隆）：后续如需 checkout 历史 commit，浅克隆不可靠。
# ---------------------------------------------------------------------------
...
# 18.06-k5.4 直接用 clone 的 master 最新提交；23.05 / master 本就用 master 最新。无需再 checkout。
case "$SRC_BRANCH" in
  openwrt-18.06-k*) echo "==> 18.06-k5.4：用 helloworld master 最新提交（xray v26.5.9 需 Go 1.26）" ;;
